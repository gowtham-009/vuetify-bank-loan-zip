<template>
  <div class="main-container w-100" :style="{height: deviceHeight + 'px'}">
    <div class="w-100 boxex-1bg-grey-lighten-4 rounded-xl d-flex justify-center align-center":style="{height:boxex1Height + 'px'}">
      <logo/>
    </div>
    <div class="w-100 boxex-2 pa-2 d-flex justify-center":style="{height:boxex2Height + 'px'}">
      <v-form class="w-100 d-flex justify-center">
    <v-container class="w-100 d-flex justify-center">
      <v-row class="d-flex flex-column justify-space-between">
        <div class="w-100">
          <v-col cols="12">
          <v-text-field label="User ID (4 digits only)" required></v-text-field>
        </v-col>
        <v-col cols="12">
          <v-text-field  label="OTP (4 digits only)" required></v-text-field>
        </v-col>
        <v-col cols="12">
          <v-text-field label="New Password (at least 6 characters)" type="password" required></v-text-field>
        </v-col>
        <v-col cols="12">
        ...
        <div class="w-100">
          ...
          <nuxt-link to="/login" style="text-decoration: none;">
            <v-btn block size="large" class="bg-indigo text-white text-h5 rounded-xl">Update</v-btn>
          </nuxt-link>
        </v-col>
        </div>
      </v-row>
    ...
    <!-- <v-dialog v-model="dialog" max-width="400">
      <v-card>
        <v-card-title>Updat Status</v-card-title>
        <v-card-text>
          {{ dialogMessage }}
        </v-card-text>
        <v-card-actions>
